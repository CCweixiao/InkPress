...
export function ReasoningBlock({
  ...
  return (
    ...
    >
      <CollapsibleTrigger className="px-2.5 py-2 text-[11px] text-violet-700 hover:bg-violet-100/50 rounded-md">
      ...
      <CollapsibleContent>
        <div className="border-t border-violet-200 px-3 py-2">
          {open && (
            <div className="whitespace-pre-wrap break-words text-[11px] leading-5 text-violet-950 font-mono">
              {text || (streaming ? "思考中…" : "")}
            </div>
          )}
        </div>
      ...
      {!open && preview && (
        <div className="px-3 pb-2 -mt-1 text-[11px] leading-5 text-violet-400/70 truncate">
          {preview}
        </div>
      )}
    </Collapsible>
